...
import 'package:flutter_absensi_app/presentation/permits/pages/add_permit_page.dart';
import 'package:flutter_absensi_app/presentation/permits/pages/attachment_viewer_page.dart';
...
class _PermitsPageState extends State<PermitsPage> {
  ...
  void initState() {
    super.initState();
    context
        .read<GetAllPermitsBloc>()
        .add(const GetAllPermitsEvent.getAllPermits());
  }
            ...
            children: [
              ...
              Expanded(
                child: Container(
                  ...
                  child: BlocBuilder<GetAllPermitsBloc, GetAllPermitsState>(
                    builder: (context, state) {
                      return state.when(
                        initial: _buildLoading,
                        loading: _buildLoading,
                        success: (permits) => _buildPermitList(permits.data ?? []),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Pantau dan atur riwayat izin Anda',
                  style: GoogleFonts.poppins(
                    fontSize: 14,
  ...
  Widget _buildPermitList(List<Permit> permits) {
    if (permits.isEmpty) {
      return _buildEmpty();
    }
    return RefreshIndicator(
      ...
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          if (index == 0) {
            return _buildSummaryCard(permits.length);
          }
          final permit = permits[index - 1];
  ...
  Widget _buildSummaryCard(int totalPermits) {
    return Container(
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      '$totalPermits request${totalPermits == 1 ? '' : 's'}',
                      style: GoogleFonts.poppins(
                        fontSize: 26,
                        fontWeight: FontWeight.w700,
                        color: Colors.white,
            ...
            child: ElevatedButton.icon(
              ...
              onPressed: () async {
                await _checkBackendAndNavigate(() async {
                  final result = await context.push(const AddPermitPage());
  ...
  Widget _buildPermitCard(Permit permit) {
    final statusLabel = _statusLabel(permit.status);
    final statusColor = _statusColor(permit.status);
    final dateRange =
        '${_formatDate(permit.startDate)} - ${_formatDate(permit.endDate)}';
    final permitType = permit.permitType?.name ?? 'Unknown Permit';
    final approver = permit.approver?.name ?? 'Awaiting assignment';
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildInfoRow(
                      ...
                      label: 'Total Days',
                      value:
                          '${permit.totalDays ?? 0} day${(permit.totalDays ?? 0) == 1 ? '' : 's'}',
                    ),
                    const SpaceHeight(12),
                    if ((permit.reason ?? '').isNotEmpty)
                      _buildInfoRow(
                        icon: Icons.notes_rounded,
                        label: 'Reason',
                        value: permit.reason!,
                      ),
                    if ((permit.reason ?? '').isNotEmpty) const SpaceHeight(12),
                    _buildInfoRow(
                      icon: Icons.verified_user_rounded,
                      label: 'Approver',
                      value: approver,
                    ),
                    if (permit.approvedAt != null) const SpaceHeight(12),
                    if (permit.approvedAt != null)
                      _buildInfoRow(
                        icon: Icons.event_available_rounded,
                        label: 'Approved At',
                        value: _formatDate(permit.approvedAt),
                      ),
                    if ((permit.attachmentUrl ?? '').isNotEmpty)
                      const SpaceHeight(12),
                    if ((permit.attachmentUrl ?? '').isNotEmpty)
                      _buildAttachmentButton(permit.attachmentUrl!),
                  ],
                ),
              ),
            ],
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SpaceHeight(8),
          Text(
            'Kirim permohonan izin untuk di tampilkan di sini',
            style: GoogleFonts.poppins(
              fontSize: 13,
              color: AppColors.black,
            ),
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              await _checkBackendAndNavigate(() async {
                await context.push(const AddPermitPage());
              });
            },
            child: Text(
